...
def llenar_encuesta(request):
    ...
    x = check_user(request)
    data['time'] = x
    if Choice.objects.filter(usuario_choice__usuario_personal = request.user).exists() == False:
        if request.POST:
            a = Personal.objects.get(usuario_personal=request.user)
            for x in request.POST:
                if x != 'csrfmiddlewaretoken':
                    if Choice.objects.filter(question_id=x, usuario_choice__usuario_personal=request.user):
                        Choice.objects.filter(question_id=x, usuario_choice=a).update(answer = request.POST[x])
                    else:
                        if request.POST[x] == 'True':
                            ...
                            Choice.objects.create(question_id=x,
                                                ...
                                                answer = True)
                        elif request.POST[x] == 'False':
                            check = Personal.objects.get(usuario_personal=request.user)
                            Choice.objects.create(question_id=x,
                                                  usuario_choice=a,
                                                  company=check.company,
                                                  answer= False)
                        elif request.POST[x] == 'None':
                            check = Personal.objects.get(usuario_personal=request.user)
                            Choice.objects.create(question_id=x,
                                                  usuario_choice=a,
                                                  company=check.company,
                                                  answer = 'No contestado')
            return HttpResponseRedirect(reverse('llenar_encuesta'))
        else:
            print('entro2')
            data["respuestas"] = Question.objects.all()
            return render(request, "question.html", data)
    else:
        #data["respuestas"] = Question.objects.all()
        if request.POST:
            for x in request.POST:
                if x != 'csrfmiddlewaretoken':
                    a = Personal.objects.get(usuario_personal=request.user)
                    if Choice.objects.all().filter(question_id=x, usuario_choice__usuario_personal=request.user):
                        print('hola 2')
                        print(Choice.objects.all().filter(question_id=x, usuario_choice=a).update(answer=request.POST[x]))
                        Choice.objects.all().filter(question_id=x, usuario_choice=a).update(answer=request.POST[x])
        data['respuestas'] = Choice.objects.all().filter(usuario_choice__usuario_personal = request.user)
        return render(request, 'question.html', data)
...
def analisis(request):
    ...
    print(respuesta)
    if Choice.objects.filter(usuario_choice__usuario_personal=request.user).exists() == True:

        respuesta2 = Question.objects.all().values('type').annotate(total=Count("choice",filter=Q(choice__company = check.company)),
                                                                    total_true=Sum(Case(
                                                                        When(choice__company = check.company,choice__answer=True, then=1.0),
                                                                        default=0,
                                                                        output_field=FloatField()
                                                                    )),
                                                                    total_false=Sum(Case(
                                                                        When(choice__company = check.company,choice__answer=False, then=1.0),
                                                                        default=0,
                                                                        output_field=FloatField()
                                                                    )))
...
def add_company(request):
    data = {}
    if request.method == 'POST':
        data['form'] = CompanyForm(request.POST)

        if data['form'].is_valid():
            data['form'].save()
            return HttpResponseRedirect(reverse('createpersonal'))

    else:
        data['form'] = CompanyForm()
    return render(request, 'createCompany.html', data)

def check_user(request):
    check = Personal.objects.get(usuario_personal = request.user)
    fecha_actual = date.today()
    y = days_between(check.initial, fecha_actual)
    x = days_between(fecha_actual, check.finish)
    if y >= 0:
        if x > 0:
            Personal.objects.filter(usuario_personal = request.user).update(status=True)
            return (x)
        elif x <= 0:
            Personal.objects.filter(usuario_personal = request.user).update(status=False)
    elif y < 0:
        Personal.objects.filter(usuario_personal=request.user).update(status=False)
        return (x)
